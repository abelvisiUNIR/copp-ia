...
from teleflow.executor_service import adapters, engine as engine_mod
...
def test_backoff_exponencial_con_jitter_y_tope(monkeypatch):
    ...
    techos = []
    monkeypatch.setattr(engine_mod.random, "uniform",
                        lambda low, high: techos.append((low, high)) or high)
    eng = ExecutionEngine(
